      ...
      <aside className="w-80 flex flex-col border-r border-white/5 bg-[#080808]">
        <div className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-black tracking-[0.3em] text-blue-500 uppercase">
              Incoming
            </h3>
            <span className="px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-400 text-[9px] font-bold">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-6 custom-scrollbar">
          {filteredLeads.length > 0 ? (
            filteredLeads.map((lead: any) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs transition-all duration-500
                  ${
                    selectedId === lead.id
                ...
                <div className="flex-1 truncate">
                  <p
                    className={`text-xs font-bold truncate transition-colors ${
                      selectedId === lead.id ? "text-white" : "text-zinc-400"
                    }`}
                  >
            ...
            <div className="text-center py-10 text-zinc-700 text-[10px] font-bold uppercase tracking-widest">
              No results found
            </div>
          )}
        </div>
            ...
            <header className="p-10 flex justify-between items-start border-b border-white/5 bg-white/[0.01]">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <h2 className="text-4xl font-black text-white tracking-tight leading-none italic">
                    {selectedLead.full_name}
                  </h2>
                  <div className="px-2 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-[8px] font-black uppercase tracking-widest">
                    Active Lead
                  </div>
                  <div className="px-2 py-1 rounded-md bg-red-500/10 border border-red-500/20 text-emerald-500 text-[8px] font-black uppercase tracking-widest">
                    {selectedLead.is_filtered ? "Verified":"Outside Area"}
                  </div>
                </div>
                <div className="flex items-center gap-4 text-zinc-500">
                  <span className="flex items-center gap-1.5 text-xs font-medium">
                    <Mail size={12} /> {selectedLead.email || "No Email"}
                  </span>
                  <span className="flex items-center gap-1.5 text-xs font-medium">
                    <Phone size={12} /> {selectedLead.phone || "No Phone"}
                  </span>
                </div>
              </div>
              ...
              <div className="max-w-3xl space-y-10">
                <section className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    />
                    <div className="absolute bottom-6 right-8 text-[10px] text-zinc-600 font-bold uppercase tracking-widest">
                      Auto-Saving Enabled
                    </div>
                  </div>
                </section>
          ...
          <div className="p-8 space-y-10">
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-zinc-600 font-medium">
                Form responses mapped from Meta Ads.
              </p>
            </div>
            ...
            <div className="space-y-4">
              {(() => {
                ...
                if (!raw?.field_data)
                  return (
                    <div className="p-10 border border-dashed border-white/5 rounded-3xl text-center text-zinc-800 text-xs">
                      No Meta Data Found
                    </div>
                  );
                ...
                return raw.field_data.map((field: any, i: number) => {
                  if (
                    ["full_name", "email", "phone_number"].includes(field.name)
                  )
                    return null;
                  ...
                  return (
                    <div
                      key={i}
                      className="group p-6 rounded-[2rem] bg-zinc-900/50 border border-white/5 hover:border-blue-500/20 transition-all overflow-hidden flex flex-col justify-center"
                    >
                      {/* Label (Small text) */}
                      <p className="text-[10px] font-black uppercase tracking-wider text-zinc-600 group-hover:text-blue-500/70 transition-colors mb-2 truncate">
                        {field.name.replace(/_/g, " ")}
                      </p>

                      {/* Value (Main text) */}
                      <p className="text-sm text-zinc-200 font-bold break-words line-clamp-2">
